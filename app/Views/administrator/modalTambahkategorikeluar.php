...
<div class="modal fade" id="modal-tambah" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <?= form_open('/auth/kategorikeluar/tambahKategori', ['class' => 'frmtambahkategori']) ?>
            <div class="modal-body">
                <?= csrf_field() ?>
                <div class="form-group row">
                    <label for="nama_kategorikeluar" class="col-sm-3 col-form-label">Nama Kategori</label>
                    <div class="col-sm-9">
                        <input type="text" class="form-control" name="nama_kategorikeluar" placeholder="Nama Kategori" id="nama_kategorikeluar">
        ...
        $('.frmtambahkategori').submit(function(e) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.error) {
                        if (response.error.nama_kategorikeluar) {
                            $('#nama_kategorikeluar').addClass('is-invalid');
                            $('.errNama').html(response.error.nama_kategorikeluar);
                        } else {
                            $('#nama_kategorikeluar').removeClass('is-invalid');
                            $('#nama_kategorikeluar').addClass('is-valid');
                            $('.errNama').html(response.error.nama_kategorikeluar);
                        }
                    } else {
                        Toast.fire({
